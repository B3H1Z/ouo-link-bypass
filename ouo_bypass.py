...
def is_ouo_url(url):
    return bool(url and ("ouo.io" in url or "ouo.press" in url))


def unique_ouo_urls(urls):
    # Keep order while removing duplicates.
    return list(dict.fromkeys(url for url in urls if is_ouo_url(url)))


async def bypass_many(urls):
    tasks = [ouo_bypass(url) for url in unique_ouo_urls(urls)]
...
async def ouo_bypass(url):
    ...
    for profile in IMPERSONATION_PROFILES:
        ...
        try:
            # Warm cookies/session with a root navigation before opening the target slug.
...
def _build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Bypass one or more ouo.io / ouo.press links."
    )
    parser.add_argument("urls", nargs="+", help="One or more ouo URLs")
    return parser


async def _run_cli(urls):
...
def main():
    ...
    raise SystemExit(asyncio.run(_run_cli(args.urls)))
